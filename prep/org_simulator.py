...
obs_beta, obs_gamma, obs_rho = 0.1, 0.05, 0.2
obs_infected_fraction, _, _ = simulate(
    beta=obs_beta, gamma=obs_gamma, rho=obs_rho, N=200, p_edge=0.05, n_infected0=5, T=200
)
print(1)
obs_summary = obs_infected_fraction[-1]  # summary: final infected fraction
prior_ranges = {
    "beta": (0.01, 0.3),
    "gamma": (0.01, 0.2),
    "rho": (0.01, 0.5),
...
def summary_stat(infected_fraction):
    return infected_fraction[-1]

def distance(sim_summary, obs_summary):
    return abs(sim_summary - obs_summary)
...
for _ in range(n_samples):
    beta = rng.uniform(*prior_ranges["beta"])
    gamma = rng.uniform(*prior_ranges["gamma"])
    rho = rng.uniform(*prior_ranges["rho"])
    infected_fraction, _, _ = simulate(
        beta=beta, gamma=gamma, rho=rho, N=200, p_edge=0.05, n_infected0=5, T=200, rng=rng
    )
    sim_summary = summary_stat(infected_fraction)
    if distance(sim_summary, obs_summary) < epsilon:
        accepted_params.append((beta, gamma, rho))

accepted_params = np.array(accepted_params)
print(f"Accepted {len(accepted_params)} / {n_samples} samples")
if len(accepted_params) > 0:
    fig, axes = plt.subplots(1, 3, figsize=(12, 4))
    axes[0].hist(accepted_params[:, 0], bins=20, color='skyblue')
    axes[0].set_title("Posterior: beta")
    axes[1].hist(accepted_params[:, 1], bins=20, color='salmon')
    axes[1].set_title("Posterior: gamma")
    axes[2].hist(accepted_params[:, 2], bins=20, color='lightgreen')
    axes[2].set_title("Posterior: rho")
    plt.tight_layout()
    plt.show()

    plt.figure(figsize=(6, 6))
    plt.scatter(accepted_params[:, 0], accepted_params[:, 1], alpha=0.5, label="beta vs gamma")
    plt.xlabel("beta")
    plt.ylabel("gamma")
    plt.title("Pairwise: beta vs gamma")
    plt.show()

    plt.figure(figsize=(6, 6))
    plt.scatter(accepted_params[:, 0], accepted_params[:, 2], alpha=0.5, label="beta vs rho")
    plt.xlabel("beta")
    plt.ylabel("rho")
    plt.title("Pairwise: beta vs rho")
    plt.show()

    plt.figure(figsize=(6, 6))
    plt.scatter(accepted_params[:, 1], accepted_params[:, 2], alpha=0.5, label="gamma vs rho")
    plt.xlabel("gamma")
    plt.ylabel("rho")
    plt.title("Pairwise: gamma vs rho")
    plt.show()

# 6. Discussion:
# - Summary statistic: final infected fraction (simple, but may lose temporal info)
# - Distance: absolute difference (normalized by tolerance)
# - Tolerance: chosen to accept ~1-5% of samples (tradeoff between accuracy and efficiency)
# - Posterior quality: If marginals are peaked near true values, ABC works well.
#   If broad, consider more informative summary stats or lower epsilon.
# - Pairwise plots show parameter correlations.